...
: config_(config),
  num_blocks_(config.block_pool_size / msg_threshold),
  round_robin_idx_(0),
...
void SharedResource::CreateControlID() {
  // Create event channel and cm id.
  ent_chan_ = rdma_create_event_channel();
  CHECK(ent_chan_ != nullptr);
  CHECK(rdma_create_id(
    ent_chan_, &cm_id_, nullptr, RDMA_PS_TCP) == 0);
}

void SharedResource::BindLocalDevice() {
  sockaddr_in addr;
  ...
  CHECK(rdma_bind_addr(
    cm_id_, reinterpret_cast<sockaddr*>(&addr)) == 0);
...
void SharedResource::PrepareSharedQueue() {
  ...
  shared_send_cq_ = ibv_create_cq(
    cm_id_->verbs, min_cqe_num, nullptr, send_chan_, 0);
  CHECK(shared_send_cq_ != nullptr);
  shared_recv_cq_ = ibv_create_cq(
    cm_id_->verbs, min_cqe_num, nullptr, recv_chan_, 0);
  CHECK(shared_recv_cq_ != nullptr);
}

void SharedResource::PrepareSharedBlockPool() {
  void* addr_ptr = mi_malloc(config_.block_pool_size);
  CHECK(addr_ptr != nullptr);
  shared_mr_ = ibv_reg_mr(
    cm_id_->pd, 
    addr_ptr, 
    config_.block_pool_size,
...
void SharedResource::StartServiceThreads() {
  ...
  for (int i = 0; i < config_.num_threads; i++) {
    ...
    work_pool_[i] = 
      new boost::asio::io_context::work(*service_pool_[i]);
    thread_pool_[i] = std::thread([this, i] {
      service_pool_[i]->run();
    });
  ...
  send_wc_processer_ = std::thread([this] {
    processer_service_.run();
  });
}

void SharedResource::StartPollerThread() {
  poller_stop_flag_ = false;
  wc_poller_ = std::thread([this] {
    if (config_.poll_mode == BUSY_POLLING)
      PollWorkCompletion();
    else BlockPollWorkCompletion();
  });
}

void SharedResource::PostOneRecvRequest(uint64_t recv_addr) {
  ibv_sge recv_sg = {
    .addr = recv_addr,
  ...
  CHECK(ibv_post_srq_recv(
    shared_rq_, &recv_wr, &recv_bad_wr) == 0);
}
...
void SharedResource::CreateQueuePair(rdma_cm_id* conn_id) {
  ...
  conn_id->qp->qp_context = reinterpret_cast<void*>(
    new QueuePairContext);
}

void SharedResource::PollWorkCompletion() {
  int send_res = 0, recv_res = 0;
  ibv_wc send_wc, recv_wc;
  while (true) {
    if (poller_stop_flag_) return;
...
void SharedResource::BlockPollWorkCompletion() {
  ...
  CHECK(fcntl(send_fd, F_SETFL, 
    send_flags | O_NONBLOCK) >= 0);
  int recv_fd = recv_chan_->fd;
  int recv_flags = fcntl(recv_fd, F_GETFL);
  CHECK(fcntl(recv_fd, F_SETFL, 
    recv_flags | O_NONBLOCK) >= 0);
  ...
  auto ProcessPollIn = [this]
    (ibv_comp_channel* comp_chan, ibv_cq* shared_cq) {
    ibv_cq* ev_cq = nullptr; 
    void* ev_ctx = nullptr;
    CHECK(ibv_get_cq_event(
      comp_chan, &ev_cq, &ev_ctx) == 0);
    CHECK(ev_cq == shared_cq);
    ibv_ack_cq_events(ev_cq, 1);
    CHECK(ibv_req_notify_cq(
      shared_cq, 0) == 0);
...
void SharedResource::ProcessSendWorkCompletion(ibv_wc& wc) {
...
void SharedResource::ProcessLargeMessage(
    ibv_qp* conn_qp, ibv_mr* msg_mr,
    uint64_t recv_addr, uint32_t msg_len, ClientOrServer flag) {
  // Get the remote info (target addr and rkey).
  RemoteInfo remote_info;
  if (msg_len >= max_threshold) {
    RemoteInfoWithId info_with_id;
    memcpy(&info_with_id, reinterpret_cast<char*>(
      recv_addr), sizeof(info_with_id));
    auto qp_context = reinterpret_cast<QueuePairContext*>(
      conn_qp->qp_context);
    std::unique_lock<std::mutex> locker(qp_context->mtx);
    qp_context->large_mr_map.insert(
      {info_with_id.rpc_id, msg_mr});
    locker.unlock();
    remote_info = {
      .remote_key = info_with_id.remote_key,
      .remote_addr = info_with_id.remote_addr
    };
  } else {
    memcpy(&remote_info, reinterpret_cast<char*>(
      recv_addr), sizeof(remote_info));
  }
  
  // For client, only return is required.
  // For server, post for next rpc call (reuse address).
  if (flag == CLIENT_FLAG) ReturnOneBlock(recv_addr);
  else PostOneRecvRequest(recv_addr);
...
ibv_mr* SharedResource::AllocLargeMsgMR(uint32_t msg_len,
                                        ibv_qp* conn_qp) {
...
ibv_mr* SharedResource::GetMemoryRegion(uint32_t rpc_id, 
                                        ibv_qp* conn_qp) {
  auto qp_context = reinterpret_cast<
    QueuePairContext*>(conn_qp->qp_context);
  ibv_mr* msg_mr = nullptr;
  std::unique_lock<std::mutex> locker(qp_context->mtx);
