...
WriteMemoryRegionInfo(ibv_qp* qp, ibv_mr* msg_mr, RemoteInfo& target) {
    ...
    RemoteInfo remote_info = {.remote_key = msg_mr->rkey,
                              .remote_addr =
                                      reinterpret_cast<uint64_t>(msg_mr->addr)};
    memcpy(info_buf, &remote_info, sizeof(remote_info));
    memset(reinterpret_cast<char*>(info_buf) + sizeof(remote_info), '1', 1);
    ibv_sge write_sg = {.addr = reinterpret_cast<uint64_t>(info_buf),
