...
void
LightServer::AddService(ServiceOwnership ownership,
...
void
LightServer::ProcessConnectRequest(rdma_cm_id* conn_id) {
...
void
LightServer::ProcessConnectEstablish(rdma_cm_id* conn_id) {
...
void
LightServer::ProcessDisconnect(rdma_cm_id* conn_id) {
    auto addr = reinterpret_cast<sockaddr_in*>(rdma_get_peer_addr(conn_id));
    CHECK(addr != nullptr);
...
void
LightServer::ProcessRecvWorkCompletion(ibv_wc& wc) {
            ...
            [this, qp_num, recv_addr, imm_data, opcode] {
                ...
                if(imm_data <= msg_threshold) {
                    ...
                    if(imm_data >= max_threshold)
                        ...
                        ParseAndRunFunc(conn_qp, msg_mr, imm_data);
                }
            });
...
void
LightServer::ParseAndRunFunc(ibv_qp* conn_qp, void* addr, uint32_t msg_len) {
    ...
    CHECK(metadata.ParseFromArray(msg_addr + req_fixed_len, metadata_size));
    ...
    uint32_t request_size = msg_len - req_fixed_len - metadata_size;
    CHECK(request->ParseFromArray(msg_addr + req_fixed_len + metadata_size,
                                  request_size));
    ...
    CallBackArgs args = {.rpc_id = rpc_id,
                         .conn_qp = conn_qp,
                         .request = request,
                         .response = response};
...
void
LightServer::ReturnRpcResult(CallBackArgs args) {
    ...
    auto SerializeFunc = [&]() {
        ...
        CHECK(args.response->SerializeToArray(msg_buf + res_head_len,
                                              response_size));
    ...
    if(msg_len <= msg_threshold) {
        ...
        mi_free(msg_buf);
        SendSmallMessage(args.conn_qp, block_addr, msg_len, shared_mr_->lkey);
    } else {
        ...
        CHECK(msg_buf != nullptr);
        SerializeFunc(); // serialize response
        ibv_mr* msg_mr = ibv_reg_mr(args.conn_qp->pd, msg_buf, alloc_len,
        ...
        if(msg_len >= max_threshold)
            call_id = args.rpc_id;
        WriteLargeMessage(args.conn_qp, msg_mr, call_id, remote_info);
    }
}
