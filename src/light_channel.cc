...
void
ClientSharedResource::ProcessRecvWorkCompletion(ibv_wc& wc) {
    ...
    service_pool_[round_robin_idx_]->post(
            [this, qp_num, recv_addr, imm_data, opcode] {
                ...
                if(imm_data <= msg_threshold) {
                    ...
                    ProcessLargeMessage(conn_qp, msg_mr, recv_addr, imm_data,
                                        CLIENT_FLAG);
                    if(imm_data >= max_threshold)
                        return;
                    else
                        ParseAndRunAfter(client, msg_mr, imm_data);
                }
            });
...
void
ClientSharedResource::ParseAndRunAfter(LightChannel* client, void* addr,
                                       uint32_t msg_len) {
    ...
    if(msg_len <= msg_threshold) {
        ...
        msg_addr =
                reinterpret_cast<char*>(reinterpret_cast<ibv_mr*>(addr)->addr);
    }
    ...
    CHECK(call_info.response->ParseFromArray(msg_addr + res_head_len,
                                             response_size));
    ...
    if(call_info.callback)
        call_info.callback->Run();
    else
        call_info.prom.set_value(true);
...
LightChannel::LightChannel(std::string dest_ip, uint16_t dest_port,
    ...
    CHECK(rdma_create_id(ent_chan_, &conn_id_, nullptr, RDMA_PS_TCP) == 0);
    ...
    local_addr.sin_addr.s_addr =
            inet_addr(shared_res_->config_.local_ip.c_str());

    CHECK(rdma_resolve_addr(conn_id_, reinterpret_cast<sockaddr*>(&local_addr),
    ...
    LOG_INFO("Connection to <IP: %s, Port: %hu> established.", dest_ip.c_str(),
             dest_port);
}

void
LightChannel::CallMethod(const google::protobuf::MethodDescriptor* method,
    ...
    auto SerializeFunc = [&]() {
        ...
        CHECK(metadata.SerializeToArray(msg_buf + req_fixed_len,
                                        metadata_size));
        CHECK(request->SerializeToArray(msg_buf + req_fixed_len + metadata_size,
                                        request_size));
    ...
    RpcCallInfo corr_info = {.prom = corr_prom,
                             .response = response,
                             .callback = done};
    ...
    if(msg_len <= msg_threshold) {
        ...
        mi_free(msg_buf);
        SendSmallMessage(conn_id_->qp, block_addr, msg_len, local_key);
    } else {
        // Send the msg length and remote info.
        char* ptr_tag =
                reinterpret_cast<char*>(block_addr) + sizeof(RemoteInfo);
        memset(ptr_tag, '0', 1);

        if(msg_len >= max_threshold) {
            RemoteInfoWithId info = {.rpc_id = corr_id,
                                     .remote_key = remote_key,
                                     .remote_addr = block_addr};
            SendNotifyMessage(conn_id_->qp, msg_len, info);
        } else {
            RemoteInfo info = {.remote_key = remote_key,
                               .remote_addr = block_addr};
            SendNotifyMessage(conn_id_->qp, msg_len, info);
        }
        ...
        if(msg_len >= max_threshold)
            ...
            alloc_len = msg_len + 1;
        msg_buf = reinterpret_cast<char*>(mi_malloc(alloc_len));
        CHECK(msg_buf != nullptr);
        SerializeFunc(); // serialize request
        ibv_mr* msg_mr = ibv_reg_mr(conn_id_->pd, msg_buf, alloc_len,
        ...
        memcpy(&remote_info, reinterpret_cast<char*>(block_addr),
               sizeof(remote_info));
        ...
        if(msg_len >= max_threshold)
            call_id = corr_id;
        WriteLargeMessage(conn_id_->qp, msg_mr, call_id, remote_info);
    }
    // For synchronous rpc, we need to wait for the response.
    if(done == nullptr)
        CHECK(corr_future.get());
}
