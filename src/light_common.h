...
public:
  ...
  void PrepareSharedQueue();
  void PrepareSharedBlockPool();
  ...
  void ProcessLargeMessage(ibv_qp* conn_qp, 
                           ibv_mr* msg_mr,
                           uint64_t recv_addr, 
                           uint32_t msg_len,
                           ClientOrServer flag);
  
  ibv_mr* AllocLargeMsgMR(uint32_t msg_len,
                          ibv_qp* conn_qp);
  ibv_mr* GetMemoryRegion(uint32_t rpc_id, 
                          ibv_qp* conn_qp);
